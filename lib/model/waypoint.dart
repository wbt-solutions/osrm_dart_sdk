...
class Waypoint {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Waypoint[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Waypoint[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Waypoint>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<Waypoint>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Waypoint>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Waypoint.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
